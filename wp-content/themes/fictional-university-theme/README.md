# Advance University WordPress Theme
Advance University WordPress theme is great for Education Institute, Events, Courses, Programs and even Professor profiles. This education WordPress theme is feature-rich but very easy to use.

Advance University comes with lots of features that a modern WordPress education theme need to have. Some of those features include: responsive design, Sliders, Custom post types for programs, events and professors.

## Features

### Theme features
- [x] Page Templates for every post type
- [x] Dynamic landing page
- [x] Cleaner menus that are accessible
- [x] Live search results that use Custom REST API
- [x] Google Maps integrated for Campuses
- [x] Notes Taking functionality for subscribers only
- [x] Professor profiles enabled and interlinked with other post types
...
[View the demo instead](http://purposep10.sg-host.com/).
...
## Usage
Clone the theme or install it with the installer that ships with [AdvanceUniversity/wordpress](https://github.com/AdvanceUniversity/wordpress). Composer is also an option.
```sh
git clone https://github.com/qasimsardar/advance-university.git
```
...
```php
<?php
namespace AdvanceUniversity\Template;

// use \AdvanceUniveristy\Media; // Optional use declarations, get image helpers or similar
...
```
// Inside another template, under namespace \AdvanceUniversity\Template
...
```php
$builder = \AdvanceUniversity\Pagebuilder::instance();
...
When the current namespace is AdvanceUniversity\Template, you can and probably should call the template functions directly, instead of going through the pagebuilder class. When working with files such as `singular.php`, use the class.
...
It's possible to build highly dynamic but maintainable components this way. You can create a generic [PostList](https://github.com/AdvanceUniversity/wordpress-theme-base/blob/master/inc/templates/PostList.php) component that uses the main query by default, but switches to a custom query if it's supplied and even supply a different (template) function.
